...
$app->post('/api/AmazonS3/getSingleObject', function ($request, $response, $args) {
    ...
    if(!empty($post_data['args']['responseCacheControl'])) {
        $body['ResponseCacheControl'] = $post_data['args']['responseCacheControl']; 
    }
    if(!empty($post_data['args']['responseContentDisposition'])) {
        $body['ResponseContentDisposition'] = $post_data['args']['responseContentDisposition']; 
    }
    if(!empty($post_data['args']['responseContentEncoding'])) {
        $body['ResponseContentEncoding'] = $post_data['args']['responseContentEncoding']; 
    }
    if(!empty($post_data['args']['responseContentLanguage'])) {
        $body['ResponseContentLanguage'] = $post_data['args']['responseContentLanguage']; 
    }
    if(!empty($post_data['args']['responseContentType'])) {
        $body['ResponseContentType'] = $post_data['args']['responseContentType']; 
    }
    if(!empty($post_data['args']['responseExpires'])) {
        $body['ResponseExpires'] = $post_data['args']['responseExpires']; 
    }
    ...
    try {
        $res = $client->getObject($body)->toArray();
